...
distancepy = 0.41		# on www.pi3dscan.com

boxwidth = 0.7		# Region with in meters
boxdepth = 0.8  			# Region depth in meters
boxheight = 1.8		# Region height, middle point is fixed based on your center targets

# Center targets
c1target = "target 1"	# 2 targets on the left side of scanner, middle height
c2target = "target 2"
...
def processscan(scanfile):
  ...
  # STEP 2 - Detect Markers
	...
	chunk.detectMarkers(PhotoScan.TargetType.CircularTarget12bit, tolerance=50)
  ...
  # STEP 3 - Create auto mask, if empty directory is specified in JSON file
	...
	if (emptydir != ""):
		...
		chunk.importMasks(maskpath, source=PhotoScan.MaskSourceBackground, operation=PhotoScan.MaskOperationReplacement, tolerance=35)
  ...
  # STEP 5 - Align Images
	chunk.matchPhotos(accuracy=PhotoScan.HighAccuracy,  generic_preselection=True, filter_mask=True, keypoint_limit=keypointLimit, tiepoint_limit=tiepointLimit,)
  ...
  # STEP 8 - Create MESH
	chunk.buildModel(surface=PhotoScan.Arbitrary, interpolation=PhotoScan.Extrapolated,  face_count = PhotoScan.HighFaceCount)
...
#		camera.photo = photo
	...
	chunk.exportModel(modelpath, binary=True, precision=6, texture_format=PhotoScan.ImageFormatJPEG, texture=True, colors= True, format=PhotoScan.ModelFormatOBJ)
